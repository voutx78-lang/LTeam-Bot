...
// Kept solely as a tiny local design-data fixture; production catalogue data
// always comes from the protected API.
void demoListings;
...
// Old visual prototypes are intentionally retained outside the live routing
// while we preserve historic deep-link compatibility during the transition.
void SettingsSheet;
void CatalogOverlay;
void CatalogPlatform;
...
function ChatSheet({ chat, currentUserId, onClose }) {
  ...
  const loadMessages = useCallback(() => apiFetch(basePath).then(setMessages).catch(() => setMessages([])), [basePath]);
  useEffect(() => { loadMessages(); const timer = window.setInterval(loadMessages, 7000); return () => window.clearInterval(timer); }, [loadMessages]);
...
export default function App() {
  const legacyCompatibility = window.location.hash === "#lteam-legacy-preview";
  ...
  return <main className="app-shell">
    ...
    {legacyCompatibility && tab === "create-order" && <OrderReferenceWidget />}
    ...
    {tab === "my-listings" && <MyListingsWorkspace fetchData={apiFetch} request={apiRequest} onNavigate={setTab} onOpenDeal={async (dealId) => { try { setDealItems(await apiFetch("/api/deals")); } catch { /* The workspace will refresh on the next polling cycle. */ } setTab("orders"); setToast(`Сделка #${dealId} создана. Откройте её в разделе «Сделки».`); }} />}
