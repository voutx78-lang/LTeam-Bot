...
function Home({ profile, listings, orders, onNavigate, isAdmin, onCreate }) {
  ...
  return <section className="pro-home">
    <header className="pro-topbar"><button className="pro-brand" onClick={() => onNavigate("home")}><Logo /><b>LT <em>Market</em></b><small>Бета</small></button><div className="pro-top-actions">{isAdmin && <button className="pro-admin-button" onClick={() => onNavigate("admin")}>Команда</button>}<button className="pro-bell" aria-label="Уведомления" onClick={() => onNavigate("profile")}><Icon name="bell" /></button><button className="pro-avatar" onClick={() => onNavigate("profile")}>{String(profile.name || "L").slice(0, 1).toUpperCase()}</button></div></header>
    ...
    <section className="home-prompt"><div><span>РАБОЧЕЕ ПРОСТРАНСТВО В TELEGRAM</span><h2>Найдите исполнителя<br/>для своей задачи</h2><p>Сравните портфолио, договоритесь об условиях и ведите работу в одном месте.</p></div><button onClick={() => onCreate()}>Создать заказ <Icon name="arrow" /></button></section>
