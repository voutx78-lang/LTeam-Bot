...
const nav = [["home", "Главная", "⌂"], ["catalog", "Каталог", "▦"], ["create-order", "Создать", "+"], ["orders", "Заказы", "▣"], ["profile", "Профиль", "○"]];
...
function Home({ profile, listings, orders, deals, onNavigate, isAdmin }) {
  ...
  return <section className="pro-home">
    ...
    <section className="pro-hero"><span className="pro-eyebrow">LTEAME MARKET · РАННИЙ ДОСТУП</span><h1>Находите сильных<br /><i>исполнителей.</i></h1><p>Каталог услуг, создание задач, обсуждение условий и история работы — в одном пространстве Telegram.</p><div className="pro-hero-actions"><button className="pro-primary" onClick={() => onNavigate("catalog")}>Найти исполнителя <b>→</b></button><button className="pro-secondary" onClick={() => onNavigate("create-order")}>Разместить заказ</button></div><div className="pro-safety"><span>✓</span><div><b>Прозрачные условия</b><small>Фиксируйте ТЗ, цену и результат в сделке. LTeam пока не принимает платежи.</small></div></div></section>
