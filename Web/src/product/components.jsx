...
export function BottomNav({ active, onNavigate, onCreate }) {
  ...
  return <nav className="bottom-nav" aria-label="Основная навигация" style={{ "--nav-index": activeIndex }}><i className="nav-indicator" aria-hidden="true"/>{items.map(([id, label, icon]) => <button key={id} className={`${active === id ? "active" : ""} ${id === "create" ? "create" : ""}`} onClick={() => id === "create" ? onCreate() : onNavigate(id)}><i><Icon name={icon} size={id === "create" ? 24 : 21}/></i><span>{label}</span></button>)}</nav>;
}
