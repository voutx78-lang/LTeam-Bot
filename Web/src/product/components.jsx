...
export function Avatar({ src, name, size = "md", verified = false }) {
  return <span className={`lt-avatar ${size}`} role="img" aria-label={name || "Пользователь"}>{src ? <img src={src} alt="" onError={(event) => { event.currentTarget.hidden = true; }} /> : null}<span>{initials(name)}</span>{verified && <i><Icon name="check" size={10} strokeWidth={3}/></i>}</span>;
}
...
export function BottomNav({ active, onNavigate, onCreate }) {
  ...
  return <nav className={`bottom-nav ${active === "create" ? "nav-create-active" : ""}`} aria-label="Основная навигация" style={{ "--nav-index": activeIndex, "--nav-shift": `${activeIndex * 100}%` }}><i className="nav-indicator" aria-hidden="true"/>{items.map(([id, label, icon]) => <button type="button" key={id} aria-current={active === id ? "page" : undefined} className={`${active === id ? "active" : ""} ${id === "create" ? "create" : ""}`} onClick={(event) => id === "create" ? onCreate("", event.currentTarget.getBoundingClientRect()) : onNavigate(id)}><i><Icon name={icon} size={id === "create" ? 23 : 20}/></i><span>{label}</span></button>)}</nav>;
}

export function ServiceCard({ item, onOpen, onFavorite, dense = false, compareSelected = false, compareDisabled = false, onCompare }) {
  return <article className={`service-card ${dense ? "dense" : ""} ${compareSelected ? "compare-selected" : ""}`} onClick={onOpen}>
    <div className="service-cover">{item.image_data ? <img src={item.image_data} alt=""/> : <span><Brand compact/></span>}{onCompare ? <button type="button" className={`compare-button ${compareSelected ? "active" : ""}`} disabled={compareDisabled && !compareSelected} onClick={(event) => { event.stopPropagation(); onCompare(item); }} aria-pressed={compareSelected} aria-label={compareSelected ? "Убрать услугу из сравнения" : "Добавить услугу к сравнению"}><Icon name={compareSelected ? "check" : "list"} size={14}/><span>{compareSelected ? "Выбрано" : "Сравнить"}</span></button> : null}<button type="button" className={`favorite-button ${item.is_favorite ? "active" : ""}`} onClick={(event) => { event.stopPropagation(); onFavorite?.(item); }} aria-label="Добавить в избранное"><Icon name="heart" size={19}/></button></div>
    <div className="service-body"><div className="seller-line"><Avatar src={item.avatar_url} name={item.seller_name || item.seller_username} size="sm" verified={Boolean(item.seller_verified)}/><div><b>{item.seller_name || item.seller_username || "Исполнитель LT"}</b><Rating rating={item.avg_rating} count={item.reviews_count} compact/></div></div><h3>{item.title}</h3><div className="service-tags"><span>{item.category}</span><span><Icon name="clock" size={14}/>{item.delivery_time || "По договорённости"}</span></div><footer><strong>{money(item.price, "от ")}</strong><button type="button" className="service-more" onClick={(event) => { event.stopPropagation(); onOpen?.(); }}>Подробнее <Icon name="chevron" size={16}/></button></footer></div>
  </article>;
}
