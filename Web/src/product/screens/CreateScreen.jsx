...
export default function CreateScreen({ initialType = "", categories, onBack, onDone, notify }) {
  ...
  return <section className="screen create-screen"><PageHeader eyebrow={type === "listing" ? "НОВАЯ УСЛУГА" : "НОВАЯ ЗАДАЧА"} title={step === totalSteps ? "Предпросмотр" : `Шаг ${step} из ${totalSteps}`} onBack={() => step > 1 ? setStep(step - 1) : onBack()}/><div className="step-progress">{Array.from({ length: totalSteps }, (_, index) => <i className={index + 1 <= step ? "active" : ""} key={index}/>)}</div>
    <div key={`${type}-${step}`} className="composer-body composer-step-enter">
      {step === 1 && <><div className={`composer-copy composer-intro ${type}`}><small>{type === "listing" ? "ВАША ВИТРИНА" : "ОСНОВА ЗАДАЧИ"}</small><h2>{type === "listing" ? "Какую услугу вы предлагаете?" : "Какого результата вы ждёте?"}</h2><p>{type === "listing" ? "Понятное предложение быстрее находит своего заказчика." : "Сформулируйте цель — мы поможем найти подходящего исполнителя."}</p><div className="composer-intro-note"><Icon name="spark" size={15}/><span>{type === "listing" ? "Чёткая услуга · больше заказов" : "Точная задача · полезные отклики"}</span></div></div><label className="field"><span>Название</span><input value={form.title} maxLength={120} onChange={(event) => update("title", event.target.value)} placeholder={type === "listing" ? "Разработаю Telegram-бота под ключ" : "Нужен дизайн Telegram-канала"}/><small>{form.title.length}/120</small></label><div className="field"><span>Категория</span><MobileSelect value={form.category} onChange={(value) => update("category", value)} options={categoryOptions} title="Выберите направление" eyebrow="КАТЕГОРИЯ" searchable/></div></>}
